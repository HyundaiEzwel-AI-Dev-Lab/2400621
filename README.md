...
아래 위치에 최종 화면 캡처를 삽입한다.

```md
![내 업무 카드형 화면](docs/images/screenshot-my-work-card.png)
![내 업무 캘린더형 화면](docs/images/screenshot-my-work-calendar.png)
```
...
Architecture Diagram은 이 섹션에 배치한다. 이미지로 관리할 경우 아래 Placeholder를 사용한다.

```md
![Architecture Diagram](docs/images/architecture-diagram.png)
```

README 내에서는 다음 Mermaid Diagram을 기준 아키텍처로 사용한다.
...
README 내에서는 다음 Mermaid ERD를 기준 구조로 사용한다.

```mermaid
erDiagram
  DEPARTMENT ||--o{ USER_ACCOUNT : has
  USER_ACCOUNT ||--o{ PROJECT_ASSIGNMENT : assigned
  PROJECT ||--o{ PROJECT_ASSIGNMENT : has
  PROJECT ||--o{ REQUIREMENT : contains
  REQUIREMENT ||--o{ WBS_TASK : contains
...
실행 후 확인 화면은 아래 위치에 삽입한다.

```md
![카드형 실행 화면](docs/images/screenshot-card-running.png)
![캘린더형 실행 화면](docs/images/screenshot-calendar-running.png)
```
